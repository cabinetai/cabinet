...
if (allowIntegrationDiff) {
  ...
  if (branch.status !== 0 || branch.stdout.trim() !== "fix/acp-provider-model-final") {
    process.stderr.write(
      "Refusing integration acceptance outside fix/acp-provider-model-final.\n",
    );
    process.exit(2);
  }
}
...
const playwrightStatus = run(
  ...
  ],
  {
    ...
    CABINET_ACCEPTANCE_PORT:
      process.env.CABINET_ACCEPTANCE_PORT ?? "4325",
