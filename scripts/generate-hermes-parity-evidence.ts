...
export async function loadExplicitProjection(args: { fixtureId?: string | null; observationsPath?: string | null; projectionUrl?: string | null; implementationRevision?: string | null; artifactGeneratedAt?: string | null }): Promise<HermesControlCenterSnapshot> {
  ...
  if (args.fixtureId) {
    if (![HERMES_ACCEPTANCE_FIXTURE_ID, HERMES_REPOSITORY_FIXTURE_ID, HERMES_RUNTIME_EXECUTION_FIXTURE_ID, HERMES_RUNTIME_INTERVENTION_FIXTURE_ID].includes(args.fixtureId)) throw new Error(`Unknown Hermes fixture ID: ${args.fixtureId}.`);
    ...
    if (args.fixtureId === HERMES_RUNTIME_INTERVENTION_FIXTURE_ID) return buildHermesRuntimeInterventionFixtureProjection({ implementationRevision: args.implementationRevision, artifactGeneratedAt: args.artifactGeneratedAt ?? null });
