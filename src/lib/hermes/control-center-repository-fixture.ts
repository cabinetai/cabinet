...
function observations(): HermesCapabilityObservation[] {
  ...
  const emptyProject = normalizeProjectObservation({ sessions: [] }, "operator-os", HERMES_REPOSITORY_FIXTURE_CAPTURED_AT);
  ...
  return [...base,
    ...
    { capabilityId: "projects", source: "Hermes session project association — empty repository scenario", interface: "/api/sessions?limit=100", observedAt: emptyProject.observedAt, assertedFreshness: "fresh", outcome: emptyProject.state, summary: emptyProject.summary, facts: { project: null, profile: emptyProject.profile, workingDirectoryReported: false, repositoryAssociated: false, repository: null }, ...proof },
