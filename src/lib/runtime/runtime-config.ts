...
/**
 * Name of the vault used when none is configured yet (fresh install) and the
 * target the one-time migration moves loose root content into. A "vault" is a
 * root cabinet: a named directory directly under the data folder. The active
 * vault's directory is the content root (see `getManagedDataDir`).
 */
export const DEFAULT_VAULT_NAME = "Cabinet";
...
/**
 * The shared data folder: the parent that holds every vault plus the state
 * that is shared across vaults (`bookmarks.json`, `.home/`, `.cabinet-state/`,
 * backups). This is the directory historically returned as DATA_DIR; the
 * content root now lives one level deeper, inside the active vault.
 */
...
let cachedActiveVault: string | null = null;

/**
 * Name of the active vault (root cabinet) for this process. Read synchronously
 * from `<dataParent>/.home/home.json` so it can seed the load-time DATA_DIR
 * const, and cached for the process lifetime — switching the active vault
...
export function getActiveVaultName(): string {
  if (cachedActiveVault) return cachedActiveVault;
  let resolved = DEFAULT_VAULT_NAME;
  try {
    ...
    // no config yet → default vault
  }
  cachedActiveVault = resolved;
  return resolved;
}

/**
 * Content root for the active vault: `<dataParent>/<activeVault>`. Exported as
 * DATA_DIR by path-utils, so the entire content/cabinet/agents tree is scoped
 * to the active vault with no per-call-site changes.
 */
export function getManagedDataDir(): string {
  return path.join(getManagedDataParentDir(), getActiveVaultName());
}

function getRuntimePortsPath(): string {
  // Runtime ports are a single-server, vault-independent concern → shared dir.
